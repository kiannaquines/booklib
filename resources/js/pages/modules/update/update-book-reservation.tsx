...
const UpdateBookReservation = ({ bookReservation, users, books }: CreateBookReservationProps) => {
    ...
    const { data, setData, reset, clearErrors } = useForm<UpdateBookReservationFormData>({
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setProcessing(true);
        router.put(route('book-reservations.update', { id: data.id }), data, {
            preserveScroll: true,
            onSuccess: () => {
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto">
                ...
                <div className="relative h-full flex-1 overflow-hidden rounded-xl border border-sidebar-border/70 md:min-h-min dark:border-sidebar-border p-5">
                    <div className="flex flex-col gap-1">
                        ...
                        <p className="text-sm text-muted-foreground">Update a book reservation in the library</p>
                    </div>
...
export default UpdateBookReservation